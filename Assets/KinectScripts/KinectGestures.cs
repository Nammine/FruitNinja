			...
			case 1:  // gesture phase 2 = complete
				if ((timestamp - gestureData.timestamp) < 1.5f) {
					bool isInPose = gestureData.joint == rightHandIndex ?
						...
						(jointsPos [rightHandIndex].z - gestureData.jointPos.z) < -0.30f :
								jointsTracked [leftHandIndex] && jointsTracked [leftElbowIndex] &&
						...
						(jointsPos [leftHandIndex].z - gestureData.jointPos.z) < -0.30f;
		...
		case Gestures.Pull:
			...
			case 0:  // gesture detection - phase 1
				if (jointsTracked [rightHandIndex] && jointsTracked [rightElbowIndex] &&
					...
					(jointsPos [leftHandIndex].y - jointsPos [leftElbowIndex].y) > -0.05f &&
					Mathf.Abs (jointsPos [leftHandIndex].x - jointsPos [leftElbowIndex].x) < 0.10f &&
			...
			case 1:  // gesture phase 2 = complete
				if ((timestamp - gestureData.timestamp) < 1.5f) {
					bool isInPose = gestureData.joint == rightHandIndex ?
								jointsTracked [rightHandIndex] && jointsTracked [rightElbowIndex] &&
						Mathf.Abs (jointsPos [rightHandIndex].x - gestureData.jointPos.x) < 0.10f && 
						Mathf.Abs (jointsPos [rightHandIndex].y - gestureData.jointPos.y) < 0.10f && 
						(jointsPos [rightHandIndex].z - gestureData.jointPos.z) > 0.15f :
								jointsTracked [leftHandIndex] && jointsTracked [leftElbowIndex] &&
						Mathf.Abs (jointsPos [leftHandIndex].x - gestureData.jointPos.x) < 0.10f &&
						Mathf.Abs (jointsPos [leftHandIndex].y - gestureData.jointPos.y) < 0.10f && 
